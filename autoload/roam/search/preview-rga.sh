...
MIMETYPE=$(file --dereference --mime-type -b "$FILE")
if [[ "$MIMETYPE" =~ application/pdf ]]; then
    true
elif [[ "${MIME:FILE_LENGTH}" =~ binary ]]; then
  echo "$MIME"
  exit 0
fi
...
if [ -z "$FZF_PREVIEW_COMMAND" ] && [ "${BATNAME:+x}" ]; then
    if [[ "$MIMETYPE" =~ application/pdf ]]; then
        rga --no-heading --color=always --smart-case "${QUERY}|$" $FILE | ${BATNAME} --style="${BAT_STYLE:-numbers}" --color=always --pager=never --highlight-line=$CENTER
    else
